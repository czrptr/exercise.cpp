...
TEST(Serde, metadata_missing)
{
  auto const bytes = [&]()
  {
    ...
    bytes.push_back(detail::serialize_raw(tag_of<Data>()));
    bytes.push_back(detail::serialize_raw(12345u));
    bytes.push_back(detail::serialize_raw('a'));
    bytes.push_back(detail::serialize_raw(tag_of<int>()));
    bytes.push_back(detail::serialize_raw(999.999));
  ...
  {
    EXPECT_STREQ("Metadata mismatch: expecting 'char' but found no metadata starting at byte 8", error.what());
  }
}
